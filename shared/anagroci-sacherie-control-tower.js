...
if(/failed to fetch|networkerror|timeout|réseau|network/.test(low))return 'Réseau indisponible. Réessayez une fois la connexion revenue.';
...
function readUrl(){try{var p=new URLSearchParams(location.search),t=p.get('tab');TAB=TABS.some(function(x){return x[0]===t;})?t:'pilotage';['cluster','rt','q','from','to','state'].forEach(function(k){FILTER[k]=p.get(k)||'';});FILTER.page=Math.max(1,Number(p.get('page'))||1);}catch(e){TAB='pilotage';}}
