/* ANAGROCI — barre de suite commune */
(function () {
  'use strict';
  ...
  var MODULES = [
    ...
    { code: 'SUS', nom: 'Sustainability',  url: 'terrain/sustainability.html', desc: 'Farmer Passport, GPS, risques et actions',     acces: ['bm', 'direction'] },
  ...
  var CSS = [
    '#anagroci-suite-bar{position:sticky;top:0;z-index:2147483001;height:' + BAR_H + 'px;display:flex;align-items:center;gap:10px;padding:0 14px;background:linear-gradient(120deg,#03301c,#053B23 60%,#0a4a2c);font-family:"IBM Plex Sans",Arial,sans-serif;box-shadow:0 2px 12px rgba(5,59,35,.32);border-bottom:2px solid #8DC556}',
    '#anagroci-suite-bar *{box-sizing:border-box}#anagroci-suite-bar a{text-decoration:none!important}',
    '.asb-logo{display:flex;align-items:center;gap:8px;color:#fff;flex:none}.asb-logo .asb-badge{width:26px;height:26px;border-radius:7px;background:#fff;display:grid;place-items:center;overflow:hidden}.asb-logo .asb-badge img{height:18px;display:block}.asb-logo b{font-size:13px;line-height:1.05;display:block}.asb-logo small{font-size:8.5px;letter-spacing:.12em;color:#8DC556;display:block}',
    '.asb-sep{width:1px;height:22px;background:rgba(255,255,255,.18);flex:none}.asb-nav{display:flex;align-items:center;gap:3px;flex:1;min-width:0;overflow:hidden}.asb-nav a{flex:none;white-space:nowrap;font-size:12.5px;font-weight:600;color:#cfe6d6;padding:6px 11px;border-radius:8px}.asb-nav a:hover{background:rgba(255,255,255,.14);color:#fff}.asb-nav a.asb-actif{background:#8DC556;color:#053B23;font-weight:700}',
    '.asb-tous{flex:none;display:flex;align-items:center;gap:6px;background:transparent;border:1px solid rgba(255,255,255,.25);color:#cfe6d6;border-radius:8px;padding:6px 10px;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit}.asb-tous:hover{background:rgba(255,255,255,.1);color:#fff}',
    '.asb-sync{flex:none;border:0;cursor:pointer;border-radius:999px;padding:6px 12px;font-size:11.5px;font-weight:700;font-family:inherit}.asb-sync.asb-ok{background:rgba(141,197,86,.2);color:#8DC556}.asb-sync.asb-wait{background:#FDEFCE;color:#8a6a12}',
    ...
    '#anagroci-suite-menu a{display:flex;gap:10px;align-items:center;padding:9px 10px;border-radius:10px;text-decoration:none!important}#anagroci-suite-menu a b,#anagroci-suite-menu a span{text-decoration:none!important}#anagroci-suite-menu a:hover{background:#F1F9EF}#anagroci-suite-menu .asb-chip{flex:none;width:36px;height:36px;border-radius:9px;display:grid;place-items:center;font-size:10.5px;font-weight:700;color:#fff;background:#0A5531;font-family:"IBM Plex Mono",Consolas,monospace}#anagroci-suite-menu a.asb-actif .asb-chip{background:#8DC556;color:#053B23}#anagroci-suite-menu b{display:block;font-size:13px;color:#053B23}#anagroci-suite-menu span.asb-d{font-size:11px;color:#6b756e}',
    '@media(max-width:1100px){.asb-nav a:nth-of-type(n+4){display:none}}@media(max-width:900px){.asb-nav a:nth-of-type(n+3){display:none}}@media(max-width:700px){.asb-nav a{display:none!important}.asb-logo small{display:none}}'
  ].join('');
  ...
  function render() {
    ...
    var menu = document.getElementById('anagroci-suite-menu');
    if (!navEl || !syncEl || !menu) return;

    menu.innerHTML = visibles.map(function (m) {
    ...
    if (sub) {
      ...
      navEl.innerHTML = '<a href="#' + sub.home.hash + '"' + (page === sub.home.hash ? ' class="asb-actif"' : '') + '>' + sub.home.label + '</a>' + sub.items.map(function (it) {
  ...
  function construire() {
    ...
    if (SHRINK) style.textContent += SHRINK + '{height:calc(100dvh - ' + BAR_H + 'px)!important;max-height:calc(100dvh - ' + BAR_H + 'px)!important}';
    document.head.appendChild(style);

    var bar = document.createElement('div');
    bar.id = 'anagroci-suite-bar';
    bar.innerHTML = '<a class="asb-logo" href="' + ROOT + 'index.html" title="Portail ANAGROCI"><span class="asb-badge"><img src="' + ROOT + 'assets/logo-pjs-mark.png" alt="PJS"></span><span><b>ANAGROCI</b><small>OPERATIONS SUITE</small></span></a><span class="asb-sep"></span><nav class="asb-nav"></nav><button type="button" class="asb-tous">Tous les modules <span style="font-size:9px">&#9660;</span></button><button type="button" class="asb-sync asb-ok">&#9679; Synchronisé</button>';
