...
-- Registre canonique : rcn_jute_locations + rcn_jute_movements.
...
create or replace function public.sacherie_ops_network_move(
  p_client_operation_id text,p_flow text,p_cluster text,p_rt_id text,p_producteur_id text,p_qty integer,
  p_receiver_name text default null,p_note text default null,p_proof_url text default null
) returns jsonb language plpgsql security definer set search_path='public','pg_temp' as $$
declare
...
create or replace function public.sacherie_ops_create_transfer(
  p_client_operation_id text,p_from_location text,p_to_location text,p_state text,p_qty integer,p_vehicle text,p_driver text,p_document_ref text,
...
declare v_scope text:=upper(coalesce(p_scope,'CAMPAIGN')); v_stock integer:=0; v_open_req integer:=0; v_open_transfers integer:=0; v_open_losses integer:=0; v_holds integer:=0; v_cluster text:=null;
begin
  if auth.uid() is null then raise exception 'Connexion requise'; end if;
  if v_scope='CLUSTER' then v_cluster:=p_code; end if;
  select coalesce(sum(abs(s.qty)),0)::integer into v_stock from public.rcn_jute_v_stock s join public.rcn_jute_locations l on l.code=s.location_code
  where s.qty<>0 and s.location_code<>'JUTE-TRANSIT' and (v_cluster is null or regexp_replace(upper(coalesce(l.cluster,'')),'[^A-Z0-9]','','g')=regexp_replace(upper(v_cluster),'[^A-Z0-9]','','g'));
  select count(*) into v_open_req from public.ops_bag_requests r where r.channel='AFLP' and r.status not in ('CLOSED','REJECTED','CANCELLED','EXPIRED','RECEIVED')
    and (v_cluster is null or regexp_replace(upper(coalesce(r.cluster,'')),'[^A-Z0-9]','','g')=regexp_replace(upper(v_cluster),'[^A-Z0-9]','','g'));
  select count(*) into v_open_transfers from public.rcn_jute_transfers t join public.rcn_jute_locations l on l.code=t.from_location
  where t.statut not in ('CLOS','ANNULE') and (v_cluster is null or regexp_replace(upper(coalesce(l.cluster,'')),'[^A-Z0-9]','','g')=regexp_replace(upper(v_cluster),'[^A-Z0-9]','','g'));
  select count(*) into v_open_losses from public.rcn_jute_loss_requests x join public.rcn_jute_locations l on l.code=x.location_code
  where x.statut='SOUMIS' and (v_cluster is null or regexp_replace(upper(coalesce(l.cluster,'')),'[^A-Z0-9]','','g')=regexp_replace(upper(v_cluster),'[^A-Z0-9]','','g'));
  select count(*) into v_holds from public.rcn_jute_inventories i join public.rcn_jute_locations l on l.code=i.location_code
  where i.reconciliation_status='HOLD' and (v_cluster is null or regexp_replace(upper(coalesce(l.cluster,'')),'[^A-Z0-9]','','g')=regexp_replace(upper(v_cluster),'[^A-Z0-9]','','g'));
