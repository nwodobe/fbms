...
const EXTRAIT_ESC = /function\s+escapeHtml\s*\(([^)]*)\)\s*\{[\s\S]*?\n\}/

for (const rel of FICHIERS.filter((f) => f.endsWith('.js'))) {
  const source = readFileSync(join(RACINE, rel), 'utf8')

  const tableau = source.match(/var ENTITIES = \{[^}]*\};/)
  const fonction = source.match(EXTRAIT_ESC)

  if (!tableau || !fonction) {
    constat('defaut', `${rel} — fonction escapeHtml introuvable`,
      'Le module doit exposer une fonction escapeHtml() unique et analysable.')
    continue
  }

  const bac = { resultat: null }
  vm.createContext(bac)
  vm.runInContext(`${tableau[0]}\n${fonction[0]}\nresultat = escapeHtml;`, bac)
...
if (defauts.length) {
  console.log('')
  console.log('Régression de sécurité : le correctif P0 du module Sacherie a été perdu.')
  process.exit(1)
}
