...
async function purchasePassport(id){
 ...
 var grn=p.reception_id?await one('wms_grns','id,status,issued_at,lot_id',function(x){return x.eq('reception_id',p.reception_id).limit(1);}).catch(function(e){return{error:e.message};}):null;
 var lotW=p.reception_id?await one('wms_lots','id,status,derogation_id,hold_reason',function(x){return x.eq('reception_id',p.reception_id).limit(1);}).catch(function(e){return{error:e.message};}):null;
 function lotFr(s){return {QUARANTINE:'Quarantaine (qualité finale en attente)',HOLD:'Bloqué (HOLD)',REQUIRES_DECISION:'Décision BM requise',REJECTED:'Rejeté',RELEASED:'Libéré',EXHAUSTED:'Épuisé',CLOSED:'Clôturé'}[s]||s||'—';}
 var grnBlock='<section class="card"><h2>GRN & LOT Warehouse — READ ONLY</h2><div class="ops-def-grid"><div><small>GRN</small><b>'+(grn&&grn.id?'<a href="warehouse.html#grn/'+encodeURIComponent(p.reception_id)+'">'+esc(grn.id)+'</a>':(grn&&grn.error?'Erreur : '+esc(grn.error):'Non émis'))+'</b></div><div><small>LOT</small><b>'+esc((lotW&&lotW.id)||'—')+'</b></div><div><small>Statut LOT</small><b>'+badge(lotW&&lotW.status?lotFr(lotW.status):(lotW&&lotW.error?'Erreur : '+lotW.error:'—'))+'</b></div><div><small>Dérogation qualité</small><b>'+esc((lotW&&lotW.derogation_id)||'—')+'</b></div></div>'+
   (lotW&&['QUARANTINE','HOLD','REQUIRES_DECISION','REJECTED'].indexOf(lotW.status)>=0?'<div class="notice danger" style="margin-top:12px"><b>Achat bloqué :</b> la soumission et l’approbation exigent un LOT libéré (qualité conforme ou dérogation tracée).</div>':'')+
   (!(grn&&grn.id)?'<div class="notice warn" style="margin-top:12px">GRN non émis : il se génère dans Warehouse, dossier de réception, après la pesée.</div>':'')+'</section>';
