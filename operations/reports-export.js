...
async function exportWorkbook(){if(!g.XLSX){alert('Moteur Excel indisponible. Rechargez la page.');return;}var sb=await waitClient();if(!sb){alert('Connexion Supabase indisponible.');return;}var btn=document.getElementById('exportXlsx');if(btn)btn.disabled=true;toast('Préparation du classeur…');try{
...
add(wb,'Metadata',[{campaign:'2027',generated_at:generated.toISOString(),source:'Supabase '+(g.ANAGROCI_SUPABASE_URL||''),schema_version:'Operations Suite MVP 2026-08-28',formula_policy:'VALUES_ONLY_NO_EXTERNAL_LINKS',note:'Les règles financières marquées À CONFIRMER ne sont pas recalculées arbitrairement.'}]);
