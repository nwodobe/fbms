...
(function () {
  ...
  function showLogin(message, kind) {
    ...
    overlay.innerHTML = box(
      ...
      '<button class="ag-primary" id="ag-btn" type="submit">Se connecter</button>' + note +
      '<p style="margin:16px 0 0;font-size:11.5px;color:#a9d9ba;text-align:center;line-height:1.5">Pas encore d\'accès ? Le <b>Branch Manager</b> crée votre compte et vous attribue vos droits.</p></form>'
    );
  ...
  function showDenied(prof) {
    overlay.innerHTML = box(
      ...
      '<p style="font-size:13.5px;color:#dcefe1;line-height:1.55;text-align:center">Bonjour <b>' + esc(prof.nom || "") + '</b>.<br>Votre rôle (<b>' + esc(prof.role || "") + '</b>) ne permet pas d\'ouvrir ce module.<br>Contactez le Branch Manager si besoin.</p>' +
  ...
  function chipHTML(prof) {
    return '<span class="ag-name">' + esc(prof.nom || prof.role) + ' · <span class="ag-role">' + esc(prof.role) + '</span></span>' + (estBM(prof.role) ? '<a class="ag-cog" href="' + adminHref() + '" title="Administration">⚙</a>' : '') + '<button class="ag-out" id="ag-logout" title="Déconnexion">⏻</button>';
  }
  function wireLogout() {
    var b = document.getElementById("ag-logout");
    b && b.addEventListener("click", function () { SB.auth.signOut().then(function () { location.reload(); }); });
  }
  function injectChip(prof) {
    var slot = document.getElementById("anagroci-userslot");
    if (slot) { slot.innerHTML = chipHTML(prof); return wireLogout(); }
  ...
  function ensureSupabase(cb) {
    ...
    sc.onerror = function () { overlay.innerHTML = box('<p style="text-align:center">Impossible de charger le module de sécurité (réseau).<br>Réessayez.</p>'); };
