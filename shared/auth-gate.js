/* ============================================================================
   ANAGROCI - PORTAIL D'AUTHENTIFICATION PARTAGE
   Verrou d'acces unique pour chaque module + injection FR/EN.
   ========================================================================== */
(function(){
  'use strict';
  ...
  var ACCESS={portail:['bm','chef','agent','direction'],fbms:['bm','chef','agent'],achats:['bm','chef','agent'],cash:['bm','chef'],sacs:['bm','chef','agent'],command:['bm','direction'],hubs:['bm','chef','agent','direction'],carte:['bm','chef','agent','direction'],audit:['bm','chef'],logistique:['bm','chef'],admin:['bm']};

  var overlay=document.createElement('div');
  overlay.id='anagroci-authgate';
  overlay.setAttribute('style',['position:fixed','inset:0','z-index:2147483647','background:radial-gradient(1000px 500px at 80% -10%,#0d6b3e,transparent 55%),linear-gradient(135deg,#03301c,#053B23 55%,#0a4a2c)','display:flex','align-items:center','justify-content:center','font-family:Segoe UI,system-ui,Arial,sans-serif','color:#eafff2','padding:20px'].join(';'));
  ...
  css.textContent='@keyframes agspin{to{transform:rotate(360deg)}}#anagroci-authgate input{width:100%;box-sizing:border-box;border:1px solid rgba(255,255,255,.25);background:rgba(255,255,255,.08);color:#fff;border-radius:10px;padding:12px 13px;font-size:15px;margin-top:6px}#anagroci-authgate input::placeholder{color:#bfe0cb}#anagroci-authgate label{display:block;text-align:left;font-size:11px;font-weight:800;letter-spacing:.5px;text-transform:uppercase;color:#bff0cf;margin-top:14px}#anagroci-authgate button.ag-primary{width:100%;margin-top:18px;border:0;border-radius:10px;background:linear-gradient(135deg,#1f8e3e,#00712C);color:#fff;font-weight:800;font-size:15px;padding:13px;cursor:pointer}#anagroci-authgate button.ag-primary:disabled{opacity:.6;cursor:default}#anagroci-userslot{display:inline-flex;align-items:center;gap:8px;vertical-align:middle}#anagroci-userslot .ag-name{font:700 12px Segoe UI,Arial,sans-serif;white-space:nowrap;max-width:190px;overflow:hidden;text-overflow:ellipsis}#anagroci-userslot .ag-role{color:#8DC556}#anagroci-userslot .ag-cog,#anagroci-userslot .ag-out{display:inline-grid;place-items:center;width:28px;height:28px;border-radius:50%;cursor:pointer;text-decoration:none;border:0;font-size:14px;flex:0 0 auto}#anagroci-userslot .ag-cog{background:#8DC556;color:#053B23;font-weight:800}#anagroci-userslot .ag-out{background:rgba(255,255,255,.18);color:#fff}#anagroci-userslot.ag-floating{position:fixed;top:12px;right:12px;z-index:2147483000;background:rgba(5,59,35,.92);color:#eafff2;border:1px solid rgba(255,255,255,.16);border-radius:999px;padding:6px 8px 6px 13px;box-shadow:0 8px 22px rgba(0,0,0,.25);backdrop-filter:blur(6px)}@media(max-width:560px){#anagroci-userslot .ag-name{max-width:96px}}';
  ...
  function showLogin(message,kind){
    ...
    setTimeout(function(){window.ANAGROCI_I18N&&window.ANAGROCI_I18N.apply();window.ANAGROCI_I18N_EXTRA&&window.ANAGROCI_I18N_EXTRA.apply()},0);
  ...
  function injectI18n(){
    if(document.getElementById('anagroci-i18n-js'))return;
    var s=document.createElement('script');s.id='anagroci-i18n-js';s.src=sharedHref('i18n.js')+'?v=132333f';s.defer=true;
    s.onload=function(){if(!document.getElementById('anagroci-i18n-extra-js')){var x=document.createElement('script');x.id='anagroci-i18n-extra-js';x.src=sharedHref('i18n-extra.js')+'?v=20260715';x.defer=true;(document.head||document.documentElement).appendChild(x)}};
  ...
  function injectAchatsCard(){if(MODULE!=='portail')return;if(document.querySelector('a[href="terrain/achats.html"]'))return;var grid=document.querySelector('main .grid')||document.querySelector('.grid');if(!grid)return;var card=document.createElement('a');card.className='card hot';card.href='terrain/achats.html';card.innerHTML='<div class="card-top"><span class="ic">ACH</span><span class="badge b-prio">Transaction</span></div><div><h2>Achats Terrain</h2><p>Saisie journalière Producteur → RT : poids net, montant payé, reçu, sacs, qualité, commission RT et file hors-ligne.</p></div><span class="open">Ouvrir le module <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg></span>';grid.insertBefore(card,grid.children[1]||null)}
  function injectAchatsDropdownPatch(){if(!/\/terrain\/achats\.html(?:$|[?#])/.test(location.pathname+location.search))return;if(document.getElementById('achats-dropdown-patch'))return;var s=document.createElement('script');s.id='achats-dropdown-patch';s.src='achats_dropdown_patch.js?v=8efe9be';document.body.appendChild(s)}
  function reveal(prof){try{injectChip(prof)}catch(e){}window.ANAGROCI_AUTH={profile:prof,niveau:niveau(prof.role),estBM:estBM(prof.role),signOut:function(){return SB.auth.signOut().then(function(){location.reload()})}};document.dispatchEvent(new CustomEvent('anagroci:authenticated',{detail:window.ANAGROCI_AUTH}));overlay.parentNode&&overlay.parentNode.removeChild(overlay);setTimeout(injectAchatsCard,0);setTimeout(injectAchatsDropdownPatch,0);setTimeout(function(){window.ANAGROCI_I18N&&window.ANAGROCI_I18N.apply();window.ANAGROCI_I18N_EXTRA&&window.ANAGROCI_I18N_EXTRA.apply()},0)}
