...
begin
  ...
  -- Compatibilité temporaire. À retirer après qualification des trois profils
  -- historiques par zone, cluster, village ou RT.
...
create or replace function private.farmer_registry_can_access_producteur(pid text)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    ...
    where p.id = pid
      and private.farmer_registry_can_access_village(p.village_id, p.rt_id)
  )
$$;

grant execute on function private.farmer_registry_authority() to authenticated;
grant execute on function private.farmer_registry_can_access_village(text,text) to authenticated;
grant execute on function private.farmer_registry_can_access_producteur(text) to authenticated;
...
create policy aflp_zones_select on public.aflp_zones
  for select to authenticated using (public.est_actif());

drop policy if exists aflp_clusters_select on public.aflp_clusters;
create policy aflp_clusters_select on public.aflp_clusters
  for select to authenticated using (public.est_actif());

drop policy if exists farmer_consents_select on public.farmer_consents;
create policy farmer_consents_select on public.farmer_consents
  for select to authenticated
  using (
    public.est_actif()
    and private.farmer_registry_can_access_producteur(producteur_id)
  );

drop policy if exists farmer_consents_insert on public.farmer_consents;
create policy farmer_consents_insert on public.farmer_consents
  for insert to authenticated
  with check (
    public.peut_editer_terrain()
...
create policy farmer_identity_documents_select on public.farmer_identity_documents
  for select to authenticated
  using (
    private.farmer_registry_can_access_producteur(producteur_id)
...
create policy farmer_identity_documents_insert on public.farmer_identity_documents
  for insert to authenticated
  with check (
    public.peut_editer_terrain()
    and created_by = auth.uid()
...
create policy farmer_identity_documents_update on public.farmer_identity_documents
  for update to authenticated
  using (public.est_bm())
  with check (public.est_bm());

drop policy if exists farmer_change_log_select on public.farmer_change_log;
create policy farmer_change_log_select on public.farmer_change_log
  for select to authenticated using (public.est_bm());

drop policy if exists farmer_registry_scope_guard on public.producteurs;
create policy farmer_registry_scope_guard on public.producteurs
  as restrictive
  for all to authenticated
  using (private.farmer_registry_can_access_village(village_id, rt_id))
  with check (private.farmer_registry_can_access_village(village_id, rt_id));

revoke all on public.aflp_zones, public.aflp_clusters from anon;
revoke insert, update, delete, truncate, references, trigger
  on public.aflp_zones, public.aflp_clusters from authenticated;
grant select on public.aflp_zones, public.aflp_clusters,
  public.farmer_passport_summary_v to authenticated;
...
revoke all on function public.farmer_registry_set_code()
  from public, anon, authenticated;
revoke all on function public.farmer_registry_prepare_producteur()
  from public, anon, authenticated;
revoke all on function public.farmer_registry_prepare_consent()
  from public, anon, authenticated;
revoke all on function public.farmer_registry_prepare_identity_document()
  from public, anon, authenticated;
revoke all on function public.farmer_registry_refresh_passport(text)
  from public, anon, authenticated;
revoke all on function public.farmer_registry_audit_trigger()
  from public, anon, authenticated;
grant execute on function public.farmer_possible_duplicates(text,text,text,text)
  to authenticated;

-- Supprime les anciens helpers publics après remplacement des politiques.
revoke all on function public.farmer_registry_authority()
  from public, anon, authenticated;
revoke all on function public.farmer_registry_can_access_village(text,text)
  from public, anon, authenticated;
revoke all on function public.farmer_registry_can_access_producteur(text)
  from public, anon, authenticated;
drop function if exists public.farmer_registry_can_access_producteur(text);
drop function if exists public.farmer_registry_can_access_village(text,text);
drop function if exists public.farmer_registry_authority();
