...
var sb=null,cache=null,loading=null,profile=null;
...
function load(){
 ...
 loading=c.auth.getSession().then(function(s){var sess=s.data&&s.data.session;if(!sess){cache=[];return [];}
  ...
  ]).then(function(x){if(x[0].error)throw x[0].error;if(x[1].error)throw x[1].error;profile=x[0].data;cache=x[1].data||[];rerender();return cache.slice();});
...
function submit(id,price,comment){if(!canSubmit())return Promise.reject(new Error("Seul le BM peut soumettre au GM."));return patch(id,{prix_bm:Number(price),commentaire_bm:comment||"",statut:"SOUMIS_GM"});}
...
function activeFor(code,at){
 ...
 return cache.filter(function(x){return x.supplier_code===code&&x.statut==="APPROUVE"&&x.valide_du<=day&&x.valide_au>=day;})
  .sort(function(a,b){return Number(b.version)-Number(a.version)||new Date(b.decided_at)-new Date(a.decided_at);})[0]||null;
}
function stats(){var a=cache||[];return {total:a.length,draft:a.filter(function(x){return x.statut==="BROUILLON"||x.statut==="A_CORRIGER";}).length,pending:a.filter(function(x){return x.statut==="SOUMIS_GM";}).length,active:a.filter(function(x){return x.statut==="APPROUVE";}).length};}
global.RCNPricing={load:load,ensure:ensure,rows:rows,role:role,canDraft:canDraft,canSubmit:canSubmit,canDecide:canDecide,create:create,submit:submit,decide:decide,close:close,activeFor:activeFor,stats:stats};
})(window);
