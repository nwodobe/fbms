...
const eventSqlPath = 'supabase/20260818_farmer_registry_phase1_event_ordering.sql';
const activeIdentitySqlPath = 'supabase/20260818_farmer_registry_phase1_active_identity_uniqueness.sql';
...
for (const file of [
  ...
  coreSqlPath, securitySqlPath, historySqlPath, eventSqlPath,
  activeIdentitySqlPath, verifySqlPath
]) {
...
const eventSql = read(eventSqlPath);
const activeIdentitySql = read(activeIdentitySqlPath);
...
assert.match(enrollment, /pendingFingerprint/);
...
assert.match(privacy, /event_order/);
assert.match(privacy, /eq\('status', 'ACTIVE'\)/);
...
assert.match(eventSql, /event_order bigint generated by default as identity/i);
assert.match(eventSql, /order by c\.event_order desc/i);
assert.match(eventSql, /order by d\.event_order desc/i);
assert.match(eventSql, /clock_timestamp\(\)/i);
assert.match(activeIdentitySql, /drop index if exists public\.farmer_identity_documents_value_uidx/i);
assert.match(activeIdentitySql, /farmer_identity_documents_one_active_uidx/i);
assert.match(activeIdentitySql, /where status = 'ACTIVE'/i);
