...
/* Navigation deleguee au routeur unique (operations/field-buying.js).
   Ce module ne definit plus sa propre barre d'onglets : deux barres
   concurrentes, c'etaient deux verites de navigation. */
function tabs(active){
  return (global.ANAGROCI_FB&&global.ANAGROCI_FB.bagNav)?global.ANAGROCI_FB.bagNav(active==='pilotage'?'overview':active==='requests'?'flows':active==='closure'?'control':'stock'):'';
}
...
/* route() et decorateBase() retires : le routage appartient desormais au
   routeur unique. */

/* Plus aucun detournement de routeur ni d'ecouteur hashchange : les rendus
   sont exposes, c'est le routeur unique de field-buying.js qui les appelle.
   Avant, deux routeurs se disputaient #bags et l'ordre de chargement des
   scripts decidait lequel gagnait. */
global.ANAGROCI_SACHERIE_P1={network:renderNetwork,transfers:renderTransfers,history:renderHistory,closure:renderClosure};
