...
(function (global) {
  ...
  PAGES.procurement = function () {
    var s = global.RCNProc ? global.RCNProc.summary() : R.procurementSummary(), eng = R.procEngagements(), fin = R.procFinancements(), arr = R.procArrivages();
    ...
    return '<div class="pagehead" style="display:flex;justify-content:space-between;align-items:flex-end;gap:18px;flex-wrap:wrap"><div><h1>Procurement · Centre de pilotage</h1><p>Avant l’arrivée du camion : combien avons-nous promis, financé et planifié ? Après la réception : combien a réellement été livré et reconnu ?</p></div>' +
      '<div class="actions" style="margin:0"><button class="btn ghost" onclick="RCNProcExport.excel()">⬇ Export Excel</button><button class="btn" onclick="RCNProcExport.pdf()">▣ Rapport PDF</button></div></div>' +
  ...
  PAGES.procfin = function () {
    ...
    var rows = R.procFinancements().map(function (f) {
      var cv = global.RCNProc ? global.RCNProc.coverage(f) : { couvertFcfa: 0, expositionFcfa: f.montant };
      var late = f.echeance && new Date(f.echeance) < new Date() && cv.expositionFcfa > 0;
      var act = f.statut === "À_APPROUVER" && R.hasPermission("finance_approve") ? '<button class="btn sm" onclick="RCNUI.approveProcFin(\'' + f.id + '\',true)">Approuver</button> <button class="btn danger sm" onclick="RCNUI.approveProcFin(\'' + f.id + '\',false)">Refuser</button>' : badgeEtat(f.statut);
      return '<tr><td class="mono">' + esc(f.id) + '</td><td><b>' + esc(f.supplierNom) + '</b><small style="display:block;color:var(--n500)">' + esc(f.engagementId || "Sans engagement") + '</small></td><td class="mono">' + money(f.montant) + '</td><td class="mono">' + money(cv.couvertFcfa) + '</td><td class="mono" style="color:' + (cv.expositionFcfa ? 'var(--danger)' : 'var(--emerald)') + '">' + money(cv.expositionFcfa) + '</td><td>' + (late ? '<span class="badge b-danger">EN RETARD</span>' : esc(f.echeance || "—")) + '</td><td>' + act + '</td></tr>';
    }).join("") || '<tr><td colspan="7" class="empty">Aucun financement enregistré.</td></tr>';
    var keys = {}, accounts = [];
    approved.forEach(function (f) { var k=f.supplierLba||f.supplierNom; if(!keys[k]){keys[k]=1;var ac=global.RCNProc?global.RCNProc.account(f.supplierLba,f.supplierNom):{financeFcfa:0,livreFcfa:0,soldeFcfa:0};accounts.push({nom:f.supplierNom,code:f.supplierLba,ac:ac});} });
    var accRows = accounts.map(function(x){return '<tr><td><b>'+esc(x.nom)+'</b><small style="display:block;color:var(--n500)">'+esc(x.code||"—")+'</small></td><td class="mono">'+money(x.ac.financeFcfa)+'</td><td class="mono">'+money(x.ac.livreFcfa)+'</td><td class="mono" style="color:'+(x.ac.soldeFcfa>0?'var(--danger)':'var(--emerald)')+'">'+money(x.ac.soldeFcfa)+'</td></tr>';}).join("") || '<tr><td colspan="4" class="empty">Aucun compte fournisseur financé.</td></tr>';
    return '<div class="pagehead"><h1>Financements LBA</h1><p>Chaque livraison est affectée une seule fois aux avances du fournisseur, dans l’ordre chronologique.</p></div>' +
      '<div class="grid2" style="align-items:start"><div class="card"><h2>Nouveau financement</h2><div class="cbody"><label>Fournisseur</label><select id="pf_supplier">' + supplierOptions() + '</select><label>Engagement lié</label><select id="pf_eng">' + engagementOptions() + '</select><div class="row">' + inp("pf_amount", "Montant (FCFA)", "", "number") + inp("pf_bank", "Banque", "", "text") + '</div><div class="row">' + inp("pf_ref", "Référence paiement", "", "text") + inp("pf_due", "Échéance livraison", "", "date") + '</div><div class="actions"><button class="btn" onclick="RCNUI.createProcFin()">Soumettre à approbation</button></div></div></div><div class="rule"><b>Contrôle renforcé.</b> Procurement prépare. Le responsable approuve. La couverture vient uniquement des livraisons libérées et rattachées aux engagements financés.</div></div>' +
      '<div class="card" style="margin-top:18px"><h2>Financements et exposition FIFO</h2><div class="cbody" style="padding:0"><div class="tablewrap"><table><thead><tr><th>FIN</th><th>Fournisseur / engagement</th><th>Financé</th><th>Couvert</th><th>Exposition</th><th>Échéance</th><th>Décision</th></tr></thead><tbody>' + rows + '</tbody></table></div></div></div>' +
      '<div class="card" style="margin-top:18px"><h2>Compte fournisseur</h2><div class="cbody" style="padding:0"><div class="tablewrap"><table><thead><tr><th>Fournisseur</th><th>Financements</th><th>Livraisons reconnues</th><th>Solde à couvrir</th></tr></thead><tbody>'+accRows+'</tbody></table></div></div></div>';
  };
  ...
  PAGES.procperf = function () {
    var secure = global.RCNSync && global.RCNSync.suppliers ? global.RCNSync.suppliers() : null;
    if (secure === null && global.RCNSync && global.RCNSync.status().hasSession) {
      global.RCNSync.loadSuppliers();
      return '<div class="pagehead"><h1>Performance fournisseurs</h1><p>Chargement des indicateurs sécurisés…</p></div><div class="card"><div class="empty">Synchronisation en cours…</div></div>';
    }
    var base = secure || [];
    var rows = base.map(function (f) {
      var js = R.juteBalance(f.code), recs = R.receptions().filter(function(r){return r.lba===f.code;});
      var rejected = recs.filter(function(r){return r.etat===R.ETAT_REC.REFUSEE||r.etat===R.ETAT_REC.BLOQUE;}).length;
      return '<tr><td><a href="#sacs/'+encodeURIComponent(f.code)+'/profil"><b>'+esc(f.nom)+'</b></a><small style="display:block;color:var(--n500)">'+esc(f.code)+'</small></td><td class="mono">'+Number(f.nb_livraisons||0)+'</td><td class="mono">'+R.kg(Number(f.volume_livre_kg||0))+'</td><td class="mono">'+(f.kor_moyen==null?"—":Number(f.kor_moyen).toFixed(2))+'</td><td class="mono">'+(f.humidite_moyenne==null?"—":Number(f.humidite_moyenne).toFixed(1)+" %")+'</td><td class="mono">'+rejected+'</td><td class="mono">'+js.solde+'</td></tr>';
    }).join("") || '<tr><td colspan="7" class="empty">Aucune donnée fournisseur accessible.</td></tr>';
    return '<div class="pagehead"><h1>Performance fournisseurs</h1><p>Volumes, qualité, blocages et dette de sacs issus de la base sécurisée.</p></div><div class="card"><h2>Comparaison fournisseurs <span class="badge b-ok">Données authentifiées</span></h2><div class="cbody" style="padding:0"><div class="tablewrap"><table><thead><tr><th>Fournisseur</th><th>Livraisons</th><th>Volume</th><th>KOR moyen</th><th>Humidité</th><th>Refus/blocages</th><th>Sacs dus</th></tr></thead><tbody>'+rows+'</tbody></table></div></div></div><div class="rule" style="margin-top:18px"><b>Prochaine validation métier.</b> La note globale restera inactive tant que les pondérations officielles ne seront pas approuvées par la direction.</div>';
  };
